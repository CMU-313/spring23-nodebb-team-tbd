...
middleware.buildHeader = helpers.try(async (req, res, next) => {
    ...
    const [config, canLoginIfBanned] = await Promise.all([controllers.api.loadConfig(req), user.bans.canLoginIfBanned(req.uid), plugins.hooks.fire('filter:middleware.buildHeader', {
        req: req,
        locals: res.locals,
    })]);
...
middleware.renderFooter = async function renderFooter(req, res, templateValues) {
    ...
    data.templateValues.scripts = scripts.map(script => ({
        src: script,
    }));
