		...
		require([
			...
			if (formatting && controls) {
				translator.getTranslations(window.config.userLang || window.config.defaultLang, 'markdown', function (strings) {
					...
					formatting.addButtonDispatch('latex', function (textarea, selectionStart, selectionEnd) {
						if (selectionStart === selectionEnd) {
							controls.insertIntoTextarea(textarea, '$' + strings.latex_text + '$');
							controls.updateTextareaSelection(
								textarea, selectionStart + 1, selectionEnd + strings.latex_text.length + 1
							);
						} else {
							var wrapDelta = controls.wrapSelectionInTextareaWith(textarea, '$', '$');
							controls.updateTextareaSelection(
